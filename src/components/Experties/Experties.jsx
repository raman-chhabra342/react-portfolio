...
import { Education, Whoami } from '../../utils/data'
    ...
    <section className={css.wrapper}>
        ...
        className={`paddings yPaddings innerWidth flexCenter ${css.container}`}>
            ...
            <div className={css.leftSide}>
                {
                    Education.map((exp, i)=> {
                        return <motion.div variants = {fadeIn("right", "tween", (i+1)*0.2, 1)} className={css.exp} key={i}>
                            ...
                            <div>
                                <span>{exp.name}</span>
                                <span className='secondaryText'>{exp.School}</span>
                                <span className='secondaryText'>Grade:{exp.cgpa}</span>
                            </div>
                        </motion.div>
                    })
                }
            ...
            className={css.rightSide}>
                ...
                {Whoami.map((paragraph, i)=> <span className='secondaryText' key={i}>{paragraph}</span>)}
